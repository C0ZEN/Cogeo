(function (angular) {
  ...
  function backState(goTo, CONFIG) {
    ...
    function addBackState(ev, to, toParams, from, fromParams) {
      if (CONFIG.debug) Methods.directiveCallbackLog('backState', 'addBackState');
      backStates.push({
        ev        : ev,
        to        : to,
        toParams  : toParams,
    ...
    function getBackStates() {
      if (CONFIG.debug) Methods.directiveCallbackLog('backState', 'getBackStates');
      return backStates;
    }

    function goToLastState() {
      if (CONFIG.debug) Methods.directiveCallbackLog('backState', 'goToLastState');
    ...
    function isBackAvailable() {
      if (CONFIG.debug) Methods.directiveCallbackLog('backState', 'isBackAvailable');
      var length = backStates.length;
      if (length == 0) return false;
      else if (backStates[length - 1].from.abstract) return false;
      return true;
    }
  }
